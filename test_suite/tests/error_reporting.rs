...
fn reports_invalid_var_with_nested_prefix() {
    ...
    unsafe {
        std::env::set_var("NAME", "api");
        std::env::set_var("SERVICE_PORT", "70000");
        std::env::set_var("SERVICE_ENABLED", "true");
        std::env::set_var("SERVICE_LETTER", "z");
    }
    ...
    assert!(matches!(
        err,
        ConfigError::InvalidVar { name, message }
            if name == "SERVICE_PORT" && !message.is_empty()
    ));
}
...
fn reports_all_missing_vars_before_parsing_present_invalid_values() {
    ...
    assert_eq!(
        err,
        ConfigError::MissingVars(vec!["ENABLED".to_string(), "LETTER".to_string()])
    );
}
